...
import org.eclipse.swt.graphics.RGB;
	 ...
	 * @see Image
	 * @see ImageLoader
	 * @see <a href="http://www.eclipse.org/swt/snippets/#image">ImageDataUpscale snippets</a>
	 * @see <a href="http://www.eclipse.org/swt/examples.php">SWT Example: ImageAnalyzer</a>
	 * @see <a href="http://www.eclipse.org/swt/">Sample code and further information</a>
	 */

	public final class ImageDataUpscale implements CloneableCompatibility {
	...
	/**
	 * Constructs a new, empty ImageDataUpscale with the given width, height,
	 ...
	 * </ul>
	 */
	public ImageDataUpscale(int width, int height, int depth, PaletteData palette) {
	 ...
	 * </ul>
	 */
	public ImageDataUpscale(int width, int height, int depth, PaletteData palette, int scanlinePad, byte[] data) {
		this(width, height, depth, palette,
	...
	/**
	 * Constructs an <code>ImageDataUpscale</code> loaded from the specified
	 ...
	 * </p>
	 * <pre>
	 *     static ImageDataUpscale loadImageData (Class clazz, String string) {
	 *          InputStream stream = clazz.getResourceAsStream (string);
	 *          if (stream == null) return null;
	 *          ImageDataUpscale imageData = null;
	 *          try {
	 *               imageData = new ImageDataUpscale (stream);
	 ...
	 */
	public ImageDataUpscale(InputStream stream) {
		ImageDataUpscale[] data = ImageDataLoaderUpscale.load(stream);
		if (data.length < 1) SWT.error(SWT.ERROR_INVALID_IMAGE);
		ImageDataUpscale i = data[0];
		setAllFields(
			i.width,
			i.height,
			i.depth,
	...
	/**
	 * Constructs an <code>ImageDataUpscale</code> loaded from a file with the
	 ...
	 * </ul>
	 */
	public ImageDataUpscale(String filename) {
		ImageDataUpscale[] data = ImageDataLoaderUpscale.load(filename);
		if (data.length < 1) SWT.error(SWT.ERROR_INVALID_IMAGE);
		ImageDataUpscale i = data[0];
		setAllFields(
			i.width,
	...
	public ImageDataUpscale( ImageData src ) {
		setAllFields(src.width, src.height, src.depth, src.scanlinePad,
				src.bytesPerLine, src.data, src.palette, src.transparentPixel,
				src.maskData, src.maskPad, src.alphaData, src.alpha,
				src.type, src.x, src.y, src.disposalMethod, src.delayTime);
	}

	public ImageData getImageData() {
		ImageData result = new ImageData(width, height, depth, palette, scanlinePad, data);
		result.maskPad = maskPad;
		result.maskData = maskData;
		result.alphaData = alphaData;
		result.alpha = alpha;
		result.transparentPixel = transparentPixel;
		result.type = type;
		result.x = x;
		result.y = y;
		result.disposalMethod = disposalMethod;
	 ...
	 * for a new ImageDataUpscale object. If a new field is added to the class,
	 ...
	 * API for <code>ImageDataUpscale</code>. It is marked public only so that it
	 ...
	 */
	public static ImageDataUpscale internal_new(
		int width, int height, int depth, PaletteData palette,
		int scanlinePad, byte[] data, int maskPad, byte[] maskData,
		byte[] alphaData, int alpha, int transparentPixel, int type,
		int x, int y, int disposalMethod, int delayTime)
	{
		return new ImageDataUpscale(
	...
	ImageDataUpscale colorMaskImage(int pixel) {
		ImageDataUpscale mask = new ImageDataUpscale(width, height, 1, bwPalette(),
	...
	/**
	 * Returns an <code>ImageDataUpscale</code> which specifies the
	 ...
	 */
	public ImageDataUpscale getTransparencyMask() {
		if (getTransparencyType() == SWT.TRANSPARENCY_MASK) {
			return new ImageDataUpscale(width, height, 1, bwPalette(), maskPad, maskData);
		} else {
	 ...
	 * associated axis.
	 *
	 * @param width the width of the new ImageDataUpscale
	 * @param height the height of the new ImageDataUpscale
	 * @return a scaled copy of the image
	 */
	public ImageDataUpscale scaledTo(int width, int height) {
		...
		ImageDataUpscale dest = new ImageDataUpscale(
	...
	static final ImageDataUpscale convertMask(ImageDataUpscale mask) {
		if (mask.depth == 1) return mask;
		PaletteData palette = new PaletteData(new RGB[] {new RGB(0, 0, 0), new RGB(255,255,255)});
		ImageDataUpscale newMask = new ImageDataUpscale(mask.width, mask.height, 1, palette);
	...
	static void blit(int op,
		...
		if ((paletteMapping != null) && (isExactPaletteMapping || ! ditherEnabled)) {
			if ((stype == dtype) && (alphaMode == 0x10000)) {
				...
				switch (stype) { // TODO Update each kind of transformer like here in TYPE_INDEX_8
					case TYPE_INDEX_8:
						int stepY;
						for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += stepY, sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
							stepY = (sfy >>> 16) * srcStride;
							int stepX;
							for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, sp += stepX, sfx = (sfx & 0xffff) + sfxi, dp += dprxi) {
								stepX = (sfx >>> 16);
								int val = srcData[sp] & 0xff;
								if( val > 0 ) { //Calculating more only if not 0, which is unmeasured value
									int spY = sp;
									int spYMax = spY + stepY;
									if( spYMax >= srcData.length )
										spYMax = srcData.length + ( sp - spr );
									int spXMaxer = stepX;
									if( spXMaxer > srcWidth - ( sp - spr ) )
										spXMaxer = srcWidth - ( sp - spr );
									for ( ; spY < spYMax; spY += srcStride ) {
										int spX = spY;
										int spXMax = spX + spXMaxer;
										for ( ; spX < spXMax; spX++ )
											if ( val < ( srcData[spX] & 0xff ) )
												val = srcData[spX] & 0xff;
									}
								}
								destData[dp] = paletteMapping[val];
							}
						}
						break;					
					case TYPE_INDEX_4:
	...
	/**
	 * Creates an ImageDataUpscale containing one band's worth of a gradient filled
	 ...
	 * @return the new ImageDataUpscale
	 */
	static ImageDataUpscale createGradientBand(
		...
		return new ImageDataUpscale(bandWidth, bandHeight, bitmapDepth, paletteData, 4, bitmapData);
	}
	...
	static void fillGradientRectangle(GC gc, Device device,
		...
		ImageData band = createGradientBand(width, height, vertical,
			fromRGB, toRGB, redBits, greenBits, blueBits).getImageData();
